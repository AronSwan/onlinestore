...
@Module({
      ...
      useFactory: () => {
        ...
        return {
          ...
          streams: {
            ...
            business_events:
              oo.streams?.events ||
              process.env.OPENOBSERVE_STREAM_BUSINESS_EVENTS ||
              'business-events',
          ...
          performance: {
            batch_size:
              oo.performance?.batchSize ??
              parseInt(process.env.OPENOBSERVE_BATCH_SIZE || '100', 10),
            flush_interval:
              oo.performance?.flushInterval ??
              parseInt(process.env.OPENOBSERVE_FLUSH_INTERVAL || '5000', 10),
            max_retries:
              oo.performance?.maxRetries ??
              parseInt(process.env.OPENOBSERVE_MAX_RETRIES || '3', 10),
            timeout:
              oo.performance?.timeout ?? parseInt(process.env.OPENOBSERVE_TIMEOUT || '30000', 10),
          },
          tracing: {
          ...
          alerts: {
            ...
            evaluation_interval:
              ...
              parseInt(process.env.OPENOBSERVE_ALERTS_EVALUATION_INTERVAL || '60', 10),
          },
        };
      },
    },
    {
      ...
      useFactory: (cfg: any) => {
        ...
        if (!OPENOBSERVE_ENABLED) {
          // 返回一个无操作传输器，避免对外部服务的依赖
          return {
            log: (_info: any, cb: () => void) => cb && cb(),
  ...
  exports: [BusinessLoggerService, UserBehaviorTracker, LogAnalyticsService],
})
