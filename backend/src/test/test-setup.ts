...
export class NestTestModuleBuilder {
  ...
  addMockProviders() {
    ...
    this.moduleBuilder = this.moduleBuilder
      .overrideProvider('OPENOBSERVE_TRANSPORT')
      .useValue(new mocks.OpenObserveTransport());

    this.moduleBuilder = this.moduleBuilder
      .overrideProvider('USER_BEHAVIOR_TRANSPORT')
      .useValue(new mocks.OpenObserveTransport());
...
export class NestTestUtils {
  ...
  static async createTestModule(): Promise<TestingModule> {
    return await new NestTestModuleBuilder().addMockProviders().compile();
  }
