...
function validateConfigAgainstSchema(config, schema) {
  ...
  // 验证必需字段
  const requiredFields = ['SEVERITY_WEIGHTS', 'SARIF_CATEGORIES', 'SARIF_RULES', 'LOCALIZATION', 'SYSTEM_MAPPING'];
...
function validateSeverityWeights(weights, result) {
  ...
  for (const severity of requiredSeverities) {
    ...
    } else if (typeof weights[severity] !== 'number' || weights[severity] < 1 || weights[severity] > 100) {
      result.valid = false;
...
function validateSystemMapping(mapping, result) {
  for (const [systemName, systemConfig] of Object.entries(mapping)) {
    ...
    if (systemConfig.priority !== undefined) {
      if (typeof systemConfig.priority !== 'number' || systemConfig.priority < 1 || systemConfig.priority > 100) {
        result.valid = false;
...
function validateSecurityConstants(options = {}) {
  ...
  try {
    ...
    // 生成报告
    const report = generateValidationReport(validationResult, config.constantsFile, config.schemaFile);
