  ...
  if (!fs.existsSync(configPath)) {
    ...
    return {
      ...
      email: {
        ...
        secure:
          String(getEmail().secure || process.env.SMTP_SECURE || 'false').toLowerCase() === 'true',
        auth: {
          user:
            getEmail().auth && getEmail().auth.user
              ? getEmail().auth.user
              : process.env.SMTP_USER || '',
          pass:
            getEmail().auth && getEmail().auth.pass
              ? getEmail().auth.pass
              : process.env.SMTP_PASS || '',
        },
        from: getEmail().from || process.env.EMAIL_FROM || '',
        to: (() => {
          const fromAdapter = Array.isArray(getEmail().to)
            ? getEmail().to
            : getEmail().to
              ? String(getEmail().to)
                  .split(',')
...
function prepareNotificationData(type, options = {}) {
  ...
  const validationData = loadReportData(
    options.validationReportFile || DEFAULT_CONFIG.validationReportFile,
  );
