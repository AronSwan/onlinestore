...
/**
 * 基础功能测试脚本 - 验证组件的基本结构和初始化
 * 避免复杂的加密和扫描逻辑，专注于测试组件的核心功能
 */
...
async function testSecurityScannerPlugin() {
  ...
  try {
    ...
    const dangerousPatterns = patterns ? patterns.dangerousFunctions || [] : [];
    
    if (dangerousPatterns && dangerousPatterns.length > 0) {
      recordTest('安全扫描模式加载', true);
    } else {
      recordTest('安全扫描模式加载', false, new Error('模式加载失败'));
    }
    ...
    if (shouldExclude) {
      ...
      recordTest('安全扫描文件排除', false, new Error('文件排除功能不正确'));
    }
...
async function runAllTests() {
  ...
  // 生成测试报告
  const reportPath = path.join(__dirname, '.test-basic-results.json');
