...
const SECURITY_RULES = {
  ...
  'jwt-secret-strength': {
    ...
    check: () => {
      ...
      if (!hasLowercase || !hasUppercase || !hasNumbers || !hasSpecialChars) {
        return { passed: false, message: 'JWT密钥复杂度不足，应包含大小写字母、数字和特殊字符', files: [] };
      }
    ...
    check: () => {
      ...
      for (const guardFile of guardFiles) {
        ...
        if (content.includes('canActivate') &&
            (content.includes('user.role') || content.includes('hasRole') || content.includes('hasAnyRole'))) {
          return { passed: true, message: '角色守卫已实现角色验证', files: [guardPath] };
        }
      }
      
      return { passed: false, message: '角色守卫未正确实现角色验证', files: guardFiles.map(f => path.join(PROJECT_ROOT, f)) };
    }
  },
  'input-length-validation': {
    ...
    check: () => {
      ...
      let passedCount = 0;
      let totalCount = dtoFiles.length;
      const files = [];
      ...
      for (const dtoFile of dtoFiles) {
        ...
        // 检查是否有长度验证装饰器
        if (content.includes('@MaxLength') || content.includes('@Length') ||
  ...
  'input-validation': {
    ...
    check: () => {
      ...
      let passedCount = 0;
      let totalCount = controllerFiles.length;
      const files = [];
  ...
  'sql-injection-protection': {
    ...
    standards: ['OWASP ASVS 5.2.2', 'OWASP ASVS 5.2.3', 'NIST SP 800-53 SI-10', 'PCI DSS 3.2.1 6.5.1'],
    check: () => {
      // 查找所有服务文件
      ...
      for (const serviceFile of serviceFiles) {
        ...
        if (content.includes('SELECT') && content.includes('${')) {
          return { passed: false, message: `发现可能的SQL注入风险: ${serviceFile}`, files: [servicePath] };
        }
  ...
  'security-headers': {
    ...
    standards: ['OWASP ASVS 10.2.1', 'OWASP ASVS 10.2.2', 'NIST SP 800-53 SC-7', 'PCI DSS 3.2.1 6.5.4'],
    check: () => {
      const mainTsPath = path.join(PROJECT_ROOT, 'src/main.ts');
      if (!fs.existsSync(mainTsPath)) {
      ...
      const hasHelmet = content.includes('helmet(') || content.includes('HelmetMiddleware') || content.includes("app.use(helmet())");
      if (hasHelmet) {
  ...
  'cors-config': {
    ...
    check: () => {
      ...
      const hasRestrictiveOrigin = /enableCors\(\{[\s\S]*origin:\s*(\[|['"]).*(?!\*)/m.test(content) && !/origin:\s*['"]\*['"]/m.test(content);
  ...
  'csrf-protection': {
    ...
    check: () => {
      ...
      const hasCsurf = hasMain.includes('csurf') || hasMain.includes('Csrf') || hasMain.includes('csrf(');
      if (hasCsurf) {
        return { passed: true, message: '检测到CSRF防护中间件', files: [mainTsPath] };
      }
      return { passed: false, message: '未检测到CSRF防护（JWT无状态API可视情况豁免）', files: [mainTsPath] };
    }
  },
  'password-hash': {
    name: '密码哈希检查',
    ...
    check: () => {
      ...
      for (const f of files) {
        ...
        if (content.includes('bcrypt.hash(') || content.includes('bcrypt.genSalt(') || content.includes('argon2') || content.includes('Argon2')) {
          usesBcrypt = true;
  ...
  'audit-logging': {
    ...
    check: () => {
      ...
      if (fs.existsSync(mainTsPath)) {
        ...
        const mainContent = fs.readFileSync(mainTsPath, 'utf8');
        registered = /useGlobalInterceptors\(new\s+AuditInterceptor\(/.test(mainContent) || mainContent.includes('AuditInterceptor');
      }
  ...
  'transaction-usage': {
    ...
    check: () => {
      ...
      for (const serviceFile of serviceFiles) {
        ...
        if (content.includes('manager.transaction') ||
  ...
  'rate-limiting': {
    ...
    check: () => {
      ...
      const content = fs.readFileSync(mainTsPath, 'utf8');
      if (content.includes('throttle') || content.includes('rateLimit') || content.includes('express-rate-limit')) {
  ...
  'ssrf-protection': {
    ...
    check: () => {
      ...
      for (const serviceFile of serviceFiles) {
        ...
        // 检查是否有外部请求
        if (content.includes('http.get') || content.includes('axios.get') || content.includes('fetch(')) {
          // 检查是否有SSRF防护
          if (content.includes('validateUrl') || content.includes('urlWhitelist') || content.includes('allowedDomains')) {
  ...
  'file-upload-security': {
    ...
    standards: ['OWASP ASVS 10.4.1', 'OWASP ASVS 10.4.2', 'NIST SP 800-53 AC-3', 'PCI DSS 3.2.1 6.5.8'],
    check: () => {
      // 查找所有文件上传相关的文件
  ...
  'path-traversal-protection': {
    ...
    check: () => {
      // 查找所有控制器和服务文件
      const files = glob.sync(['src/**/*.controller.ts', 'src/**/*.service.ts'], { cwd: PROJECT_ROOT, nodir: true });
      const filePaths = [];
      ...
      for (const file of files) {
        ...
        if (content.includes('fs.') ||
          ...
          // 检查是否有路径遍历防护
          if (content.includes('path.normalize') ||
