          ...
          rules: Object.keys(SECURITY_RULES).map(id => {
            ...
            return {
              ...
              defaultConfiguration: {
                level: ((SECURITY_RULES[id]?.severity) || 'medium') === 'critical' ? 'error' :
                       ((SECURITY_RULES[id]?.severity) || 'medium') === 'high' ? 'error' : 'warning'
              }
            };
          })
        }
            ...
            if (fs.existsSync(filePath)) {
              ...
              switch (r.id) {
                ...
                case 'jwt-format-validation':
                  for (let i = 0; i < lines.length; i++) {
                    if (lines[i].includes('extractTokenFromHeader') || lines[i].includes('Bearer')) {
                      lineNumber = i + 1;
                      break;
                    }
                  }
                ...
                case 'password-hash':
                  for (let i = 0; i < lines.length; i++) {
                    if (lines[i].includes('bcrypt.hash(') || lines[i].includes('bcrypt.genSalt(')) {
                      lineNumber = i + 1;
                      break;
                    }
                  }
...
function shouldFail(results, failOn) {
  ...
  return threshold == null
    ...
    : failedResults.some(r => (SEVERITY_WEIGHT[r.severity || (SECURITY_RULES[r.id]?.severity) || 'medium'] || 2) >= threshold);
}
