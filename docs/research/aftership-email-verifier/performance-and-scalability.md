...
AfterShip Email Verifier 在设计上充分考虑了性能因素，采用了多种优化策略来确保高效的邮箱验证。本文基于源码分析（verifier.go、smtp.go、mx.go、schedule.go、handler.go）与变更记录（CHANGELOG），聚焦于真实实现细节与生产优化建议。
...
#### 1.1 DNS缓存
- MX记录查询结果缓存，避免重复的DNS查询
- 使用sync.Map实现线程安全的缓存机制

#### 1.2 元数据缓存
- 一次性邮箱域名缓存（disposableSyncDomains sync.Map）
- 免费邮箱域名缓存（内存驻留map结构）
- 元数据体量较大（metadata_disposable.go ~3.1MB），需评估内存占用

### 2. 并发处理优化

#### 2.1 连接池管理
- SMTP连接复用，避免频繁创建销毁连接
- 并发拨号策略：对所有MX记录并发尝试连接，首个成功者胜出

#### 2.2 并发安全
- 线程安全的验证器实例（sync.RWMutex）
- 适当的锁机制，避免竞态条件

### 3. 超时控制策略

#### 3.1 双重超时机制
- **ConnectTimeout**（默认10s）：建立TCP连接的上限
- **OperationTimeout**（默认10s）：SMTP命令读写的截止时间

#### 3.2 代理超时协同
- 代理URI支持timeout参数（如`socks5://proxy:1080?timeout=5s`）
- 与Verifier超时参数协同调优

## 关键性能行为与耗时来源

### 1. SMTP验证性能分析
- **并发拨号**：smtp.go的newSMTPClient对所有MX记录并发尝试连接，第一个成功的连接胜出并关闭其他连接
- **catch-all检测**：启用时额外进行一次RCPT TO随机地址，引入额外往返
- **用户名校验**：非catch-all情况下对具体邮箱做RCPT，决定Deliverable
- **代理开销**：使用x/net/proxy ContextDialer，带来额外连接建立成本

### 2. 其他验证模块性能
- **MX查询**：net.LookupMX单次查询快，但高并发时需考虑缓存
- **语法/元数据验证**：CPU内存低成本操作
- **Gravatar查询**：网络往返开销，启用时需考虑性能影响
