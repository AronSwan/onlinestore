...
const CONFIG = {
    ...
    logStreams: ['application-logs', 'system-metrics', 'request-traces']
};
...
function calculateStatistics() {
    ...
    testResults.p99ResponseTime = testResults.responseTimes[Math.floor(testResults.responseTimes.length * 0.99)];
...
function generateReport() {
    ...
    log(`P99响应时间: ${testResults.p99ResponseTime.toFixed(2)}ms`);
    log('');
    ...
    if (testResults.errors.length > 0) {
        ...
        if (testResults.errors.length > 10) {
            log(`... 还有 ${testResults.errors.length - 10} 个错误`, colors.red);
        }
    }
    
    // 性能评估
    log('\n=== 性能评估 ===', colors.cyan);
    if (testResults.errorRate < 1 && testResults.p95ResponseTime < 2000) {
        log('✅ 性能测试通过', colors.green);
    } else if (testResults.errorRate < 5 && testResults.p95ResponseTime < 5000) {
...
async function runWritePerformanceTest() {
    ...
    return testResults.errorRate < 1 && testResults.p95ResponseTime < 2000;
}
...
async function runQueryPerformanceTest() {
    ...
    const queries = [
        'SELECT * FROM application-logs WHERE timestamp >= now() - INTERVAL \'1 hour\' LIMIT 100',
        'SELECT level, COUNT(*) FROM application-logs WHERE timestamp >= now() - INTERVAL \'1 day\' GROUP BY level',
        'SELECT * FROM system-metrics WHERE metric_name = \'cpu_usage_percent\' ORDER BY timestamp DESC LIMIT 100',
        'SELECT time_bucket(\'5 minutes\', timestamp) as bucket, AVG(CAST(value AS DOUBLE)) FROM system-metrics WHERE metric_name = \'memory_usage_percent\' AND timestamp >= now() - INTERVAL \'1 hour\' GROUP BY bucket'
    ];
    ...
    if (queryResults.responseTimes.length > 0) {
        ...
        return queryErrorRate < 1 && p95QueryTime < 1000;
    }
...
async function monitorSystemResources() {
    ...
    try {
        ...
        // 检查磁盘使用
        const diskUsage = execSync('df -h | grep -E "/$|/data"', { encoding: 'utf8' });
        log('磁盘使用情况:', colors.cyan);
        log(diskUsage);
...
async function runPerformanceTests() {
    log('=== OpenObserve性能测试 - 阶段四优化版本 ===', colors.cyan);
